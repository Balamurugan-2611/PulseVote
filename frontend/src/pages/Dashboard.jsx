...
import { PollCardSkeleton, Skeleton, StatsSkeleton } from '../components/ui/LoadingSkeleton';
...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return { totalPolls: polls.length, totalVotes: allVotes, activePolls: active.length };
  }, [polls]);

  const spotlight = useMemo(
    () => polls.filter((p) => p.status === 'active').sort((a, b) => totalVotes(b.options) - totalVotes(a.options))[0],
    [polls]
  );
  ...
  const visiblePolls = useMemo(
    () => polls.filter((p) => {
      ...
      const matchesQuery = p.question.toLowerCase().includes(query.trim().toLowerCase());
  ...
  return (
    <div className="flex min-h-full w-full flex-col bg-canvas">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8 sm:px-6 lg:py-10">
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
            ...
            {!ready ? (
              ...
              <Card className="flex h-full flex-col">
                <div className="flex items-center justify-between gap-3">
                  ...
                  >
                    <RadioIcon className="h-4 w-4 text-live-500" aria-hidden="true" />
                    Live now
                  </h2>
                ...
                <div className="mt-6 flex flex-wrap gap-2 border-t border-line pt-5">
                  <Link to={`/polls/${spotlight.id}/results`} className={buttonClasses('primary', 'sm')}>
                    Open live results
                    <ArrowRightIcon className="h-4 w-4" aria-hidden="true" />
                  </Link>
                  <Link to={`/polls/${spotlight.id}`} className={buttonClasses('secondary', 'sm')}>
                    Manage poll
                  </Link>
                </div>
              </Card>
          ...
          <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
            {!ready ? (
              <div className="sm:col-span-3 lg:col-span-1"><StatsSkeleton /></div>
            ) : (
              <>
                <StatsCard label="Total polls" value={String(stats.totalPolls)} icon={<ListChecksIcon className="h-4 w-4" aria-hidden="true" />} />
                <StatsCard label="Total votes" value={String(stats.totalVotes)} icon={<UsersIcon className="h-4 w-4" aria-hidden="true" />} />
                <StatsCard label="Active polls" value={String(stats.activePolls)} icon={<BarChart3Icon className="h-4 w-4" aria-hidden="true" />} footnote="Accepting votes right now" />
              </>
            )}
          </div>
        </div>
        ...
        <section aria-labelledby="recent-activity" className="mt-10">
          ...
          {!ready ? (
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <PollCardSkeleton /><PollCardSkeleton />
            </div>
      ...
      <ClosePollModal
        ...
        onConfirm={async () => {
          if (pollToClose) {
            try { await closePoll(pollToClose.id); } catch { /* toast shown in PollCard */ }
          }
          setPollToClose(null);
        }}
      />
