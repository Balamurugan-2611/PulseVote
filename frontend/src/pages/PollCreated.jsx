    ...
    <div className="flex min-h-full w-full flex-col bg-canvas">
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-10 sm:px-6 lg:py-14">
        ...
        <section
          ...
          className="mt-8 rounded-xl border border-line bg-surface p-5 shadow-card sm:p-6"
        >
          ...
          <ul className="mt-4 flex flex-wrap gap-2">
            {poll.options.map((option) => (
              <li
                key={option.id}
                className="rounded-full border border-line bg-raised px-3 py-1.5 text-xs font-semibold text-ink-muted"
              >
                {option.label}
              </li>
            ))}
      ...
      <Modal
        open={qrOpen}
        title="Poll QR code"
        description="Project this on a screen so people in the room can join instantly."
