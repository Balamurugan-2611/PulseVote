    ...
    >
      <div className="rounded-xl border border-line bg-surface p-6 shadow-card sm:p-8">
        ...
        <form className="mt-6 space-y-4" onSubmit={submit} noValidate>
          <Input label="Full Name" autoComplete="name" placeholder="Your Name"
            value={name} error={errors.name} onChange={(e) => setName(e.target.value)} />
          <Input label="Email" type="email" autoComplete="email" placeholder="you@company.com"
            value={email} error={errors.email} onChange={(e) => setEmail(e.target.value)} />
          <PasswordInput label="Password" autoComplete="new-password" placeholder="Create a password"
            value={password} error={errors.password} hint="At least 8 characters."
            onChange={(e) => setPassword(e.target.value)} />
          <PasswordInput label="Confirm Password" autoComplete="new-password" placeholder="Re-enter your password"
            value={confirm} error={errors.confirm} onChange={(e) => setConfirm(e.target.value)} />

          <Button type="submit" size="lg" fullWidth loading={loading} loadingLabel="Creating account…">
            Create Account
          </Button>
        </form>
