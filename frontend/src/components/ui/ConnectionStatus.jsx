...
export function ConnectionStatus({ note = 'Results update automatically', className, size = 'md' }) {
  ...
  if (connectionState === 'connecting') {
    return (
      <span
        role="status"
        className={twMerge(
          'inline-flex items-center gap-2 rounded-full border border-warn-100 bg-warn-50 font-semibold text-warn-500',
          pad,
          className
        )}
      >
  ...
  return (
    <span
      role="status"
      className={twMerge(
        'inline-flex items-center gap-2 rounded-full border border-danger-100 bg-danger-50 font-semibold text-danger-500',
        pad,
        className
      )}
    >
      ...
      Connection lost
      <span className="font-medium text-danger-500/70">Trying to reconnect…</span>
    </span>
  );
}
