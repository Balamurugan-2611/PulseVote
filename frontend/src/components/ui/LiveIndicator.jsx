...
export function LiveIndicator({ label = 'LIVE', note, size = 'md', className }) {
  return (
    <span
      className={twMerge(
        'inline-flex items-center gap-2 rounded-full border border-live-100 bg-live-50 font-semibold tracking-wide text-live-500',
    ...
    >
      ...
      {label}
      {note ? (
        <span className="font-medium normal-case tracking-normal text-live-600">{note}</span>
      ) : null}
    </span>
  );
}
