...
export function ShareCard({
  ...
  description = 'Anyone with this link can participate in your poll. Results will update live as people vote.',
}) {
  ...
  const share = async () => {
    ...
    if (shareApi) {
      try {
        await shareApi({ title: 'Vote in my PulseVote poll', url });
        return;
      } catch {
        /* user dismissed */
      }
  ...
  return (
    <section aria-label={title} className="rounded-xl border border-line bg-surface p-5 sm:p-6">
      <div className="flex items-center gap-2">
        <LinkIcon className="h-4 w-4 text-brand-400" aria-hidden="true" />
        <h2 className="text-sm font-bold uppercase tracking-wider text-ink">{title}</h2>
      </div>

      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <label className="sr-only" htmlFor="share-url">Poll link</label>
        <input
          id="share-url"
          readOnly
          value={url}
          onFocus={(event) => event.currentTarget.select()}
          className={twMerge(inputClasses, 'font-medium text-ink-muted sm:flex-1')}
        />
