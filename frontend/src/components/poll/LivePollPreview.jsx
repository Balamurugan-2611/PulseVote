...
export function LivePollPreview({ compact = false }) {
  ...
  useEffect(() => {
    if (!liveUpdates || connectionState !== 'connected') return undefined;
    let timer = 0;
    const schedule = () => {
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-line bg-surface p-5 shadow-lift sm:p-6"
    >
      ...
      <div className="mt-5">
        <LiveResultsPanel
          options={options}
          emphasis={compact ? 'compact' : 'default'}
          updatedOptionId={updated?.id ?? null}
          updateKey={updated?.key ?? 0}
        />
      </div>
          ...
          <AnimatePresence>
            {updated ? (
              <motion.span
                ...
                className="absolute right-0 text-xs font-semibold text-live-500"
              >
                +1 vote
              </motion.span>
            ) : null}
